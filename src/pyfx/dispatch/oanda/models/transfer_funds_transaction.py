...
class TransferFundsTransaction(BaseModel):
    """
    ...
    type: Optional[StrictStr] = Field(None, description="The Type of the Transaction. Always set to \"TRANSFER_FUNDS\" in a TransferFundsTransaction.")
    amount: Optional[StrictStr] = Field(None, description="The amount to deposit/withdraw from the Account in the Account's home currency. A positive value indicates a deposit, a negative value indicates a withdrawal.")
    funding_reason: Optional[StrictStr] = Field(None, alias="fundingReason", description="The reason that an Account is being funded.")
    comment: Optional[StrictStr] = Field(None, description="An optional comment that may be attached to a fund transfer for audit purposes")
    account_balance: Optional[StrictStr] = Field(None, alias="accountBalance", description="The Account's balance after funds are transferred.")
    additional_properties: Dict[str, Any] = {}
    __properties = ["id", "time", "userID", "accountID", "batchID", "requestID", "type", "amount", "fundingReason", "comment", "accountBalance"]
    ...
    @validator('funding_reason')
    def funding_reason_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in ('CLIENT_FUNDING', 'ACCOUNT_TRANSFER', 'DIVISION_MIGRATION', 'SITE_MIGRATION', 'ADJUSTMENT'):
            raise ValueError("must be one of enum values ('CLIENT_FUNDING', 'ACCOUNT_TRANSFER', 'DIVISION_MIGRATION', 'SITE_MIGRATION', 'ADJUSTMENT')")
        return value
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> TransferFundsTransaction:
        """Create an instance of TransferFundsTransaction from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return TransferFundsTransaction.parse_obj(obj)

        _obj = TransferFundsTransaction.parse_obj({
            "id": obj.get("id"),
            "time": obj.get("time"),
            ...
            "funding_reason": obj.get("fundingReason"),
