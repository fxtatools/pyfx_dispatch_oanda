...
class OrderFillTransaction(InstrumentTxn, OrderIdentifier):
    ...
    type: Literal[TransactionType.ORDER_FILL] = TransportField(TransactionType.ORDER_FILL)
    """
    The Type of the Transaction. Always set to \"ORDER_FILL\" for an OrderFillTransaction.
    """

    requested_units: Optional[LotsValue] = TransportField(None, alias="requestedUnits")
    ...
    gain_quote_home_conversion_factor: Optional[FloatValue] = TransportField(None, alias="gainQuoteHomeConversionFactor", deprecated=True)
    """
    ...
    loss_quote_home_conversion_factor: Optional[FloatValue] = TransportField(None, alias="lossQuoteHomeConversionFactor", deprecated=True)
    """
    ...
    price: Optional[PriceValue] = TransportField(None, deprecated=True)
    """
    ...
    full_vwap: Optional[PriceValue] = TransportField(None, alias="fullVWAP")
    """
    ...
    full_price: Optional[ClientPrice] = TransportField(None, alias="fullPrice")
    """
    The price in effect for the account at the time of the Order fill.
    """

    reason: Optional[OrderFillReason] = TransportField(None)
    """
    ...
    quote_pl: Optional[FloatValue] = TransportField(None, alias="quotePL")
    """
    ...
    trade_opened: Optional[TradeOpen] = TransportField(None, alias="tradeOpened")
    """
    The Trade that was opened when the Order was filled (only provided if filling the Order resulted in a new Trade).
    """

    trades_closed: Optional[list[TradeReduce]] = TransportField(None, alias="tradesClosed")
    """
    ...
    trade_reduced: Optional[TradeReduce] = TransportField(None, alias="tradeReduced")
    """
    ...
    guaranteed_execution_feed_home_conversion_cost: Optional[FloatValue] = TransportField(None, alias="guaranteedExecutionFeeHomeConversionCost")
    """
    This field is supplemental to the fxTrade v20 API 3.0.25 and not documented at the developer hub
    """
